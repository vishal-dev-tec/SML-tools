...
def main():
    ...
    parser.add_argument(
        ...
        help="Adaptive mode: run a fast triage probe, then let a local small "
             "LLM (via Ollama) pick which remaining tools fit this target, "
             "instead of running everything. Ignores -t when set.",
    )
    parser.add_argument(
        "--ai-model", default=ai_planner.DEFAULT_MODEL,
        help=f"Ollama model tag for adaptive mode (default: {ai_planner.DEFAULT_MODEL}).",
    )
    ...
    if args.ai:
        triage_keys = ai_planner.ALWAYS_RUN + ai_planner.TRIAGE_ONLY
        print(f"[*] Target: {args.target}")
        print(f"[*] AI adaptive mode: running triage probe ({len(triage_keys)} tools) first...\n")
        triage_results = run_suite(args.target, triage_keys, outdir, max_workers=args.workers, progress_cb=progress)
        triage_by_key = {r.tool_key: r for r in triage_results}

        print(f"\n[*] Asking {args.ai_model} which tools fit this target...")
        planned_keys, planner_note = ai_planner.plan_tools(triage_by_key, model=args.ai_model)
        print(f"[*] {planner_note}\n")

        final_keys = [k for k in planned_keys if k not in triage_keys]
        print(f"[*] Running {len(final_keys)} AI-selected tool(s) with up to {args.workers} in parallel...\n")
        remaining_results = run_suite(args.target, final_keys, outdir, max_workers=args.workers, progress_cb=progress)
        results = triage_results + remaining_results
    else:
        print(f"[*] Target: {args.target}")
