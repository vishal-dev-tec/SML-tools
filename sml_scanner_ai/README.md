...
## AI Adaptive Mode (small local LLM picks the tools)

Instead of running the full 22-tool suite blind, `--ai` runs a fast triage
probe first (whois, dig, httpx, nmap top-1000 ports — ~1-2min), feeds the
findings to a small local model via Ollama, and only runs the tools the
model judges relevant to what's actually there (e.g. skips `wpscan` if it's
not WordPress, skips `enum4linux` if SMB isn't open, skips
`web3_rpc_probe` if there's no sign of an RPC service).

```bash
ollama pull qwen3:0.6b   # ~400-500MB at Q4_K_M, fits well under your 700MB budget
python3 -m sml.scanner https://target.tld -y --ai
```

Qwen3's 0.6B size is part of the official Qwen3 lineup (0.6B-235B) and was
specifically trained with agentic/tool-use capability in mind, which is
exactly this job — picking the right tool for the situation rather than
generating text. Swap models with `--ai-model`:

```bash
python3 -m sml.scanner target.tld -y --ai --ai-model qwen2.5:0.5b-instruct
```

**Failure handling**: if Ollama isn't running or the model output can't be
parsed, the planner logs why and falls back to a fixed, sane default tool
set (`dnsrecon, nmap_tcp, whatweb, nikto, gobuster, nuclei, sslscan,
testssl`) — the scan never silently does nothing. Whichever path was taken
is recorded on the PDF's cover page under "AI Adaptive Mode."

`-t` is ignored when `--ai` is set — the model chooses `-t` for you.
